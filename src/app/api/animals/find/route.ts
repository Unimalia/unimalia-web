...
function toPublicAnimal(row: AnimalRow) {
  return {
    id: row.id,
    name: row.name,
    species: row.species,
  ...
  try {
    if (id) {
      ...
      return NextResponse.json(
        { found: true, animal: toPublicAnimal(animal) },
        { status: 200 }
      );
    }
    ...
    if (isUuid(q)) {
      ...
      if (byId) {
        return NextResponse.json(
          { found: true, animal: toPublicAnimal(byId) },
          { status: 200 }
        );
      }
      ...
      return NextResponse.json(
        { found: true, animal: toPublicAnimal(byCode) },
        { status: 200 }
      );
    }
