...
export default function CollegaProprietarioPage() {
  ...
  async function handleInviteSubmit(e: React.FormEvent) {
    ...
    if (!email.trim()) {
      setErr("Inserisci lâ€™email del proprietario.");
      return;
    }
    ...
    <main className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Usa lo stesso animale giÃ  creato dalla clinica, senza duplicare la scheda.
          </p>
        </div>
      ...
      <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-2 text-sm">
          ...
          <div>
            ...
            <div className="font-semibold text-zinc-900">
              {animal.microchip ? normalizeChip(animal.microchip) : "â€”"}
            </div>
          </div>
      ...
      <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm space-y-4">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
          In questa fase puoi inviare al proprietario unâ€™email per collegare lo stesso animale al suo account.
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <button
            type="button"
            onClick={() =>
              copyValue("Link identitÃ ", `${window.location.origin}${identityPath}`)
            }
            className="rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-left text-sm font-semibold text-zinc-800 hover:bg-zinc-50"
          >
            Copia link identitÃ 
